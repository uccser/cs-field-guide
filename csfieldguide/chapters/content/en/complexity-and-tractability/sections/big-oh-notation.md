# Asymptotic Complexity (Big-Oh Noatation)
...
A more formal way of writing this is using "big-oh" notation, such as \( O(n) \), which is a quick and simple way to characterise an algorithm based on the number of values it has to process.
...
The time taken is proportional to \( n \), and this is simplified to \(O(n)\) in "big-oh" notation.
Even if an algorithm takes \( 3n \) steps, we write it as \(O(n)\), since the time taken will be proportional to \( n \).

The big-oh notation is called "asymptotic" complexity because "asymptote" means what happens as \( n \) gets very large.
