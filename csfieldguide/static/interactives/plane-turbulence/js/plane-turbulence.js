...
function addInstruction() {
    instruction_text = currentInstruction().description;
    if (instruction_text) {
        instruction = document.createElement("div");
        instruction.appendChild(document.createTextNode(instruction_text));
...
/**
* Marks the top instruction as complete.
*/
function markInstructionComplete() {
    instructions.firstElementChild.classList.add("completed");
}


/**
* Marks all instructions as complete.
* Useful when an instruction has multiple instructions appear sequentially
* and all need to be marked off at once.
*/
function markAllInstructionsComplete() {
    all_instructions = instructions.children;
    for (var i = 0; i < all_instructions.length; i++) {
        all_instructions[i].classList.add("completed");
    }
}
