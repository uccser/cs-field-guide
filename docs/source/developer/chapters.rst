...
The chapters application is the main focus of the CS Field Guide website, as it
...
In general the chapters application is a standard Django application, however it
...
The content for the chapters application is stored within the ``contents/``
...
The management command for updating the applications data is ``loadchapters``
(which is automatically called when running ``updatedata``), and can be found at
``management/commands/loadchapters.py``.
