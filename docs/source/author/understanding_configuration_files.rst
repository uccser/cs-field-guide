...
Here is an example configuration file used to define the order of sections in a chapter 
in the CS Field Guide project:
...
- Inside this dictionary, the key ``section-number`` stores an integer indicating
  the number order for this section. For example searching will be the first section, 
  sorting the second and so on.
