...
"use strict";
var Observable, _, animationFrames, corruptString, drawCenteredText, drawPacket, eventStream, packetTCP,
...
packetTCP = require('./packets.coffee').packetTCP;
...
drawCenteredText = function(ctx, opts) {
  ...
  return ctx.fillText(opts.text, centerX, centerY);
};

drawPacket = function(ctx, packet, time, renderOptions) {
  var cornerX, cornerY, distance, i, idx, left, location, packetState, ref, ref1, ref2, results, segmentWidth, text;
  ...
  if ((ref = packetState.packetType) === 'ack' || ref === 'nack') {
    cornerY = ctx.canvas.height * 2 / 3;
  } else {
    cornerY = ctx.canvas.height * 1 / 3;
  }
  if (packetState.packetType === 'nack') {
    ctx.fillStyle = 'red';
  } else if (packetState.packetType === 'ack') {
    ctx.fillStyle = 'green';
  }
  segmentWidth = distance * packet.bandwidth;
  if (packetState.packetType === 'packet') {
    results = [];
    for (idx = i = 0, ref1 = packet.contentLength; 0 <= ref1 ? i < ref1 : i > ref1; idx = 0 <= ref1 ? ++i : --i) {
      ctx.fillStyle = 'white';
      ctx.fillRect(cornerX + idx * segmentWidth, cornerY, segmentWidth, renderOptions.size);
      ctx.strokeRect(cornerX + idx * segmentWidth, cornerY, segmentWidth, renderOptions.size);
      text = indexOf.call(packetState.corruptIndices, idx) >= 0 ? '?' : packet.contentValue[idx];
      results.push(drawCenteredText(ctx, {
        text: text,
    ...
    return results;
  } else if ((ref2 = packetState.packetType) === 'ack' || ref2 === 'nack') {
    ctx.fillRect(cornerX, cornerY, segmentWidth, renderOptions.size);
    return ctx.strokeRect(cornerX, cornerY, segmentWidth, renderOptions.size);
  }
};

(function() {
  var SPEED, canvas, canvasID, clockViewID, ctx, packet;
  SPEED = 1 / 20;
  packet = packetTCP({
    errRate: 0.2,
    contentType: "string",
...
},{"./observable.coffee":3,"./packets.coffee":4,"es5-shim":5,"es6-shim":6,"underscore":7}],3:[function(require,module,exports){
...
},{"zen-observable":8}],4:[function(require,module,exports){
"use strict";
var _, choice, geometricRandomNumber, packetDefaults, packetState, packetTCP, randomIndices,
...
packetTCP = function(opts) {
  var ackTripTime, bandwidth, baseResult, contentLength, contentType, contentValue, errRate, latency, packetTripTime, ref, ref1, ref2, ref3, ref4, ref5, roundTripTime, roundTrips, start, successChance;
  if (opts == null) {
    opts = {};
  }
   ...
   */
  errRate = (ref = opts.errRate) != null ? ref : packetDefaults.errRate;
  start = (ref1 = opts.start) != null ? ref1 : packetDefaults.start;
  latency = (ref2 = opts.latency) != null ? ref2 : packetDefaults.latency;
  ...
  contentValue = (ref5 = opts.contentValue) != null ? ref5 : packetDefaults.contentValue;
  ...
  ackTripTime = latency * bandwidth + latency;
    ...
    return _.extend(baseResult, {
      ...
      state: function(time) {
        return {
          ...
          sent: time - start > 0,
          corruptIndices: [],
          roundTrip: modulo(time, roundTripTime),
          location: 1
        };
      }
    });
  ...
  return _.extend(baseResult, {
    ...
    state: function(realTime) {
      ...
      /* This returns the state of a packet at a given point in time,
          ...
          rendering
       */
      var ackTime, location, packetType, roundTrip, time, tripTime;
      time = realTime - start;
      if (time < 0) {
        return {
          sent: false,
      ...
      roundTrip = Math.floor(time / roundTripTime);
      ...
      tripTime = modulo(time, roundTripTime);
...
},{"./zen-observable.js":9}],9:[function(require,module,exports){
(function (process,global){
